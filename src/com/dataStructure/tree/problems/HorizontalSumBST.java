...
public class HorizontalSumBST {
	...
	public void printHorizontalSum(Node root, int order) {
		if(root == null)
			return;

		map.put(order, (map.containsKey(order)?map.get(order):0) + root.getData());
		printHorizontalSum(root.getLeft(), order+1);
		printHorizontalSum(root.getRight(), order+1);
	}
	...
	public static void main(String[] args) {
		...
		HorizontalSumBST bst = new HorizontalSumBST();
		bst.printHorizontalSum(root, 0);
